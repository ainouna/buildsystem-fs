...
flash-image-$(BOXTYPE)-multi-rootfs:
	...
	echo $(BOXTYPE)_DDT_usb_$(shell date '+%d%m%Y-%H%M%S') > $(IMAGE_BUILD_DIR)/$(IMAGEDIR)/imageversion
	cd $(IMAGE_BUILD_DIR) && \
	zip -r $(RELEASE_IMAGE_DIR)/$(BOXTYPE)_multi_disk_ofg_$(shell date '+%d.%m.%Y-%H.%M').zip $(IMAGEDIR)/rootfs.tar.bz2 $(IMAGEDIR)/kernel.bin $(IMAGEDIR)/disk.img $(IMAGEDIR)/imageversion
	# cleanup
	rm -rf $(IMAGE_BUILD_DIR)
