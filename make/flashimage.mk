...
# disk image
flash-image-h7-disk-image:
	# Create final USB-image
	mkdir -p $(IMAGE_BUILD_DIR)/$(H7_IMAGEDIR)
	cd $(RELEASE_DIR); \
	echo $(BOXTYPE)_DDT_usb_$(shell date '+%d%m%Y-%H%M%S') > $(IMAGE_BUILD_DIR)/$(H7_IMAGEDIR)/imageversion
	cd $(IMAGE_BUILD_DIR) && \
	zip -r $(RELEASE_IMAGE_DIR)/$(BOXTYPE)_multi_disk_img_$(shell date '+%d.%m.%Y-%H.%M').zip $(H7_IMAGEDIR)/disk.img $(H7_IMAGEDIR)/imageversion
	# cleanup
	rm -rf $(IMAGE_BUILD_DIR)
...
flash-image-h7-multi-rootfs:
	...
	echo $(BOXTYPE)_DDT_usb_$(shell date '+%d%m%Y-%H%M%S') > $(IMAGE_BUILD_DIR)/$(H7_IMAGEDIR)/imageversion
	cd $(IMAGE_BUILD_DIR) && \
	zip -r $(RELEASE_IMAGE_DIR)/$(BOXTYPE)_multi_disk_ofg_$(shell date '+%d.%m.%Y-%H.%M').zip $(H7_IMAGEDIR)/rootfs.tar.bz2 $(H7_IMAGEDIR)/kernel.bin $(H7_IMAGEDIR)/disk.img $(H7_IMAGEDIR)/imageversion
	# cleanup
	rm -rf $(IMAGE_BUILD_DIR)
